...
- 没有报错直接告诉用户无异常，不要增加
- 用户明确要查更多 → 500 行
- 500 行还没有 → 让用户提供具体时间或关键词，用 grep 搜索

### 没有报错时的标准回复
如果日志里没有 ERROR/Exception/WARN，直接回复：
> 当前 {env} 环境 {service} 服务最近 100 行日志无异常，服务运行正常。
> 如需查看更多日志或搜索特定关键词，请告诉我具体时间范围或错误描述。

### 禁止行为
- 禁止自行增加 lines 参数超过 500
- 禁止自行编写任何新脚本
- 禁止在没有明确指示的情况下查其他服务的日志
